...
def test_defaults_are_per_model_and_image_means_caption_mode() -> None:
    assert und.default_questions(JobKind.IMAGE_UNDERSTAND) == (None, None)
    audio, no_second = und.default_questions(JobKind.AUDIO_UNDERSTAND)
    video, track = und.default_questions(JobKind.VIDEO_UNDERSTAND)
    ...
    with pytest.raises(ValueError):
        und.default_questions(JobKind.VIDEO)
...
async def test_no_text_is_the_default_and_calls_no_llm() -> None:
    ...
    q, aq, how = await und.convert_question("  ", client, modality=JobKind.AUDIO_UNDERSTAND)
    assert how == "understanding-default" and q == und.AUDIO_DEFAULT_QUESTION and aq is None
    q, aq, how = await und.convert_question("", client, modality=JobKind.VIDEO_UNDERSTAND)
...
async def test_text_without_a_client_is_sent_as_typed() -> None:
    q, aq, how = await und.convert_question("這是誰", None, modality=JobKind.IMAGE_UNDERSTAND)
    assert (q, aq, how) == ("這是誰", None, "understanding-raw")
    q, aq, how = await und.convert_question("他說了什麼", None, modality=JobKind.VIDEO_UNDERSTAND)
...
def test_compose_answer_joins_the_video_sections_and_notes_silence() -> None:
    common = dict(shot_id="j", provider="p", job_id="1", modality=JobKind.VIDEO_UNDERSTAND)
    ...
    plain = UnderstandingAsset(result_text="a dog", **{**common, "modality": JobKind.IMAGE_UNDERSTAND})
...
async def test_a_question_is_rewritten_with_the_models_own_system_prompt() -> None:
    ...
    q, _, how = await und.convert_question("這是什麼品種的狗", client, modality=JobKind.IMAGE_UNDERSTAND)
    assert how == "understanding-llm"
    assert q == "What breed is the dog in the photo?"
    system, user = client.calls[0]
    assert system is und.REWRITE_SYSTEM[JobKind.IMAGE_UNDERSTAND]
...
async def test_a_bad_reply_is_retried_then_falls_back_to_the_users_words_labelled() -> None:
    ...
    q, _, how = await und.convert_question("他在唱什麼", client, modality=JobKind.AUDIO_UNDERSTAND)
    assert q == "他在唱什麼"
...
async def test_a_second_attempt_that_succeeds_is_a_retry() -> None:
    ...
    q, aq, how = await und.convert_question("最後怎麼了", client, modality=JobKind.VIDEO_UNDERSTAND)
...
async def test_a_transport_failure_falls_back_rather_than_raising() -> None:
    ...
    q, _, how = await und.convert_question("誰在說話", _Down(), modality=JobKind.AUDIO_UNDERSTAND)
    assert q == "誰在說話" and "ConnectionError" in how


def test_rewriter_system_prompts_name_the_models_language() -> None:
    assert "only in English" in und.REWRITE_SYSTEM[JobKind.IMAGE_UNDERSTAND]
    for kind in (JobKind.AUDIO_UNDERSTAND, JobKind.VIDEO_UNDERSTAND):
        assert "請只用繁體中文" in und.REWRITE_SYSTEM[kind]
    # /說影 hears too (a second model on the track); the rewriter must know.
    assert "listens to\nits audio track" in und.REWRITE_SYSTEM[JobKind.VIDEO_UNDERSTAND]
    assert "cannot hear" not in und.REWRITE_SYSTEM[JobKind.VIDEO_UNDERSTAND]
