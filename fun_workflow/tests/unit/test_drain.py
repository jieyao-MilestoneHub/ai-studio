...
async def test_requeue_is_capped_so_a_broken_pod_cannot_loop_forever(tmp_path: Path) -> None:
    ...
    from fun_workflow.pipeline.drain import MAX_ATTEMPTS
...
async def test_the_breaker_stops_the_window_after_three_failures_in_a_row(
    ...
    from fun_workflow.pipeline.drain import MAX_CONSECUTIVE_FAILURES
...
async def test_the_clip_is_as_long_as_the_prompt_was_planned_for(tmp_path: Path) -> None:
    ...
    from fun_workflow.pipeline.convert_worker import DEFAULT_FRAMES
...
def test_frame_counts_snap_up_to_the_17k_plus_5_grid() -> None:
    from fun_workflow.pipeline.convert_worker import MIN_FRAMES, snap_frames
...
async def test_drain_window_dispatches_a_drama_without_a_providers_entry_for_it(tmp_path: Path, monkeypatch) -> None:
    ...
    from fun_workflow.pipeline import drain as drain_mod
