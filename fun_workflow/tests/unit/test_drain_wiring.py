...
from fun_workflow.cli.main import app
...
def test_drain_is_a_real_command() -> None:
    """Without this the window is a bill with no product."""
    result = runner.invoke(app, ["drain", "--help"])
...
def test_drain_constructs_both_the_video_and_image_provider(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    monkeypatch.setattr("fun_workflow.cli.main.get_provider", _fake_get_provider)

    result = runner.invoke(app, ["drain"])
    assert result.exit_code == 0, result.output
    assert set(requested) == {
        "comfyui", "flux", "understand-image", "understand-audio", "understand-video", "chat",
    }, "drain must build a provider for every media_kind the queue can hold"


def test_the_vps_runbook_documents_manual_drain_recovery() -> None:
    """`drain` is off every timer now -- the worker renders each parsed
    ...
    """
    script = (REPO / "fun_workflow" / "deploy" / "vps_setup.sh").read_text(encoding="utf-8")
    assert "funapp drain" in script, (
...
def test_the_claim_deadline_is_the_live_pods_lease_or_a_fresh_one(
    ...
    from fun_workflow.cli.main import _RuntimeHost
