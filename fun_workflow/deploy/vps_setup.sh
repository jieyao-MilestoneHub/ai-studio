#!/usr/bin/env bash
# Provision a fresh Ubuntu box as the always-on side of fun_workflow (on ai-studio).
#
...
sudo -u "$APP_USER" env PATH=/usr/local/bin:/usr/bin:/bin \
  sh -c "cd $APP_DIR/fun_workflow && uv sync" || echo "  uv sync failed - check the checkout"
...
ExecStart=/usr/local/bin/uv run --project ${APP_DIR}/fun_workflow funapp line serve --host 127.0.0.1 --port 8000
Restart=always
RestartSec=5
...
WorkingDirectory=${APP_DIR}
Environment=PATH=/usr/local/bin:/usr/bin:/bin
ExecStart=/usr/local/bin/uv run --project ${APP_DIR}/fun_workflow funapp worker
Restart=always
...
WorkingDirectory=${APP_DIR}
Environment=PATH=/usr/local/bin:/usr/bin:/bin
ExecStart=/usr/local/bin/uv run --project ${APP_DIR}/fun_workflow ${cmd}
UNIT
...
cat <<NEXT
  ...
  7. (cd fun_workflow && uv run funapp preflight); uv run ai-studio preflight

  If the worker ever wedges while a pod is open, empty the queue by hand with
  'uv run --project fun_workflow funapp drain' -- it claims nothing when no pod is up, so
