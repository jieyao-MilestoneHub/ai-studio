...
# the persistent side of fun_workflow (on ai-studio), with ngrok as the public
# HTTPS front.
#
...
say "dependencies (both packages, one venv under fun_workflow/)"
# fun_workflow installs ai-studio editable from `..`, so this one sync gives
# every unit below both console scripts: funapp and ai-studio.
sudo -u "$APP_USER" env PATH="$UNIT_PATH" sh -c "cd $APP_DIR/fun_workflow && uv sync" \
  || { echo "  uv sync failed in fun_workflow/"; exit 1; }
...
ExecStart=${BIN}/uv run --project ${APP_DIR}/fun_workflow funapp line serve --host 127.0.0.1 --port 8000
Restart=always
RestartSec=5
...
Environment=HOME=${APP_HOME}
Environment=PATH=${UNIT_PATH}
ExecStart=${BIN}/uv run --project ${APP_DIR}/fun_workflow funapp worker
Restart=always
...
for phase in reap close gc archive; do
  case "$phase" in
    ...
    close) cmd="ai-studio session close"; when="04:05 Asia/Taipei" ;;
    ...
    # quiet hour.
    gc)    cmd="funapp gc";            when="02:30 Asia/Taipei" ;;
    ...
    archive) cmd="funapp archive";     when="03:00 Asia/Taipei" ;;
  esac
...
Environment=HOME=${APP_HOME}
Environment=PATH=${UNIT_PATH}
ExecStart=${BIN}/uv run --project ${APP_DIR}/fun_workflow ${cmd}
UNIT
...
cat <<NEXT
  ...
  6. cd ${APP_DIR}/fun_workflow && uv run funapp preflight   (and: cd ${APP_DIR} && uv run ai-studio preflight)
