...
4. **T v2 前的訓練資料處置（2026-08-30 由 T v1 煙霧測試發現）**：(a) `train/formatting.py` 的 `ensure_ascii` 已修；(b) 本人回覆若只是 LINE 媒體佈位符（「圖片」「貼圖」「影片」），目前照原文進 `ActionStep.content`（實測 260/19,976 筆回覆為純媒體佈位符：貼圖 162、圖片 60、多行純媒體 35、影片 3，僅 1.3%，所以 T 反覆回「圖片」主要是退化生成而非資料占比）——需裁決：排除這些回覆、或轉成如 `[media:image]` 的明確標記讓 L4 能對應到真實動作；(c) 訪談軌跡（`ingest/interview_trajectories.py`）納入訓練集。
