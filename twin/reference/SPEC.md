...
| D39 | B1 persona／B2 逐字稿 MAY 以雲端 GPU 函式的**輸入參數**過境（Modal `.remote()`：經 Modal 後端輸入暫存 → 容器記憶體／ephemeral 磁碟），MUST NOT 寫入任何 Volume、bucket 或跨雲中樞（2026-08-30） | 只在本機 GPU 跑 B2（本機無可用 GPU）；或放棄 B2 | 沒有 B2 就沒有 kill switch（`EVAL.md` §3.4）；過境不是儲存，但確實離開本機，故以裁決承載而非解讀 §6.3 | 自陳內容經雲端函式輸入外洩（§8 D32 路徑）；緩解：不落 Volume/R2、`generate_s1_candidates.py` 只寫容器 tmp |
