...
| Disk | 📏 13.8GB — the three `model-0000*-of-00002.safetensors` shards its index maps to, into the standard HF cache, pre-staged by `deploy/pod_setup.sh`'s `dl_repo openai/gpt-oss-20b 'original/*' 'metal/*'` | the full repo is 41.3GB; `original/` (13.8GB raw MXFP4 for the reference implementation) and `metal/` (13.8GB, Apple) are never read by `transformers`. Counted in that script's ~182GB disk-headroom check, so a short volume fails at setup rather than inside the first `/himonkey` request |
