"""Image-to-video: a photo sent before `生成` becomes the H3 first frame.
...
- the webhook pairs a cached photo with the *next* video trigger from the
  same sender, and only the next one
...
async def test_a_photo_then_a_trigger_becomes_the_first_frame(tmp_path: Path) -> None:
    ...
    (accept_outcome,) = await _send(handler, _text_event("生成 貓在下雨的窗邊"))
...
async def test_a_photo_is_not_reused_after_being_claimed(tmp_path: Path) -> None:
    ...
    await _send(handler, _text_event("生成 貓", event_id="evt-1"))
    (second,) = await _send(handler, _text_event("生成 狗", event_id="evt-2"))

    assert second.job is not None and second.job.first_frame_path is None
    queue.close()


@pytest.mark.asyncio
async def test_the_image_trigger_does_not_consume_a_pending_photo(tmp_path: Path) -> None:
    """Flux (畫圖) has no first-frame concept -- a photo meant for the next
    生成 must not be silently eaten by an intervening 畫圖."""
    content = NullContentClient(b"fake-jpeg-bytes")
    handler, queue, _ = _wired(tmp_path, content=content)

    await _send(handler, _image_event())
    (img_job_outcome,) = await _send(handler, _text_event("畫圖 一隻貓", event_id="evt-1"))
    ...
    (video_outcome,) = await _send(handler, _text_event("生成 一隻貓", event_id="evt-2"))
    assert video_outcome.job is not None and video_outcome.job.first_frame_path is not None
    queue.close()
...
async def test_a_stale_photo_is_not_used(tmp_path: Path) -> None:
    ...
    (outcome,) = await _send(handler, _text_event("生成 貓"))

    assert outcome.job is not None and outcome.job.first_frame_path is None
    queue.close()
...
async def test_a_photo_from_one_user_is_not_claimed_by_another(tmp_path: Path) -> None:
    ...
    (outcome,) = await _send(handler, _text_event("生成 貓", user=USER))
