"""Drain the queue while the service window is open.

The loop is shaped by two facts about the window rather than by the work itself:

1. **A clip takes about five minutes and the window ends on a clock.** So the
   drainer stops *claiming* new work well before the bell and only finishes what
   it already holds. Starting a render at 12:58 means paying for four minutes of
   GPU and then having `--terminate-after` throw the result away — money spent
   for nothing, which is worse than making someone wait for tomorrow.
