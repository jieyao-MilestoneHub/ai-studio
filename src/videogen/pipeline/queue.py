...
`queued` means the webhook accepted it; `parsed` means the LLM turned it into a
validated H3 prompt and it is ready for a GPU. Only `parsed` jobs are claimable,
so a request whose prompt could not be built never occupies window time.
"""
...
class JobQueue:
    ...
    def set_parsed(self, job_id: int, prompt: dict[str, Any]) -> Job | None:
        """Attach a validated H3 prompt and make the job claimable."""
        cur = self._conn.execute(
    ...
    def unparsed(self, limit: int = 50) -> list[Job]:
        """Jobs the LLM has not converted yet — the background worker's input."""
        rows = self._conn.execute(
