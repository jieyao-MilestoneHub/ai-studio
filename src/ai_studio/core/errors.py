...
class ConfigError(AIStudioError):
    """Settings or policy file is missing, malformed, or has an unknown key."""


class UnknownKeyError(AIStudioError):
...
class FormatPolicyViolation(AIStudioError):
...
class GraphValidationError(AIStudioError):
...
class GateFailure(AIStudioError):
...
class CostCeilingExceeded(AIStudioError):
